...
class ChatFilter {
private:
    static inline std::vector<std::regex> s_regexes;
    ...
    static void init() {
        ...
        for (const auto& str : encoded) {
            ...
            s_regexes.push_back(std::regex("\\b" + escaped + "\\b", std::regex_constants::icase));
        }
        s_initialized = true;
    }
...
public:
    static std::string filter(const std::string& input) {
        if (!geode::Mod::get()->getSettingValue<bool>("chat-filter")) {
            return input;
        }
        ...
        for (const auto& re : s_regexes) {
            std::smatch m;
            std::string temp_result;
            std::string::const_iterator searchStart( result.cbegin() );
            while (std::regex_search(searchStart, result.cend(), m, re)) {
                temp_result.append(searchStart, m[0].first);
                temp_result.append(m[0].length(), '*');
                searchStart = m[0].second;
            }
            temp_result.append(searchStart, result.cend());
            result = temp_result;
        }
        return result;
    }
