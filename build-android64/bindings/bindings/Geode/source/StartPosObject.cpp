...
StartPosObject::~StartPosObject() {
	...
	if (!geode::DestructorLock::isLocked(this)) {
		using FunctionType = void(*)(StartPosObject*);
		static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN14StartPosObjectD2Ev")), tulip::hook::TulipConvention::Default);
		reinterpret_cast<FunctionType>(func)(this);

		// we need to construct it back so that it uhhh ummm doesnt crash
		// while going to the child destructors
		auto thing = new (this) StartPosObject(geode::CutoffConstructor, sizeof(StartPosObject));
...
auto StartPosObject::customObjectSetup(gd::vector<gd::string>& values, gd::vector<void*>& exists) -> decltype(customObjectSetup(values, exists)) {
	...
	static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN14StartPosObject17customObjectSetupERSt6vectorISsSaISsEERS0_IPvSaIS4_EE")), tulip::hook::TulipConvention::Default);
...
auto StartPosObject::loadSettingsFromString(gd::string objectString) -> decltype(loadSettingsFromString(objectString)) {
	...
	static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN14StartPosObject22loadSettingsFromStringESs")), tulip::hook::TulipConvention::Default);
