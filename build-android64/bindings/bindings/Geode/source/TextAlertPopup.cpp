...
auto TextAlertPopup::create(gd::string text, float delay, float scale, int opacity, gd::string font) -> decltype(create(text, delay, scale, opacity, font)) {
	...
	static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN14TextAlertPopup6createESsffiSs")), tulip::hook::TulipConvention::Default);
...
auto TextAlertPopup::init(gd::string text, float delay, float scale, int opacity, gd::string font) -> decltype(init(text, delay, scale, opacity, font)) {
	...
	static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN14TextAlertPopup4initESsffiSs")), tulip::hook::TulipConvention::Default);
