...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GooglePlayManager> : ModifyBase<ModifyDerive<Der, GooglePlayManager>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getNonVirtual(Resolve<>::func(&GooglePlayManager::sharedState)), Default, GooglePlayManager, sharedState, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&GooglePlayManager::init)), Default, GooglePlayManager, init, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getNonVirtual(Resolve<>::func(&GooglePlayManager::googlePlaySignedIn)), Default, GooglePlayManager, googlePlaySignedIn, )
		}
	};
}
