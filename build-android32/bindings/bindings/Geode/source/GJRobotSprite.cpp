...
auto GJRobotSprite::init(int frame, gd::string animName) -> decltype(init(frame, animName)) {
	...
	static auto func = wrapFunction<FunctionType>(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN13GJRobotSprite4initEiSs")), tulip::hook::TulipConvention::Default);
